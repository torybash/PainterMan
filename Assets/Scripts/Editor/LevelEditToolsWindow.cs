...
public class LevelEditToolsWindow : EditorWindow {

	[SerializeField] private List<Level> levelList;

	private Vector2 scrollPos;

	[MenuItem("Window/LevelEdit Window")]
    static void Init() {
		LevelEditToolsWindow window = (LevelEditToolsWindow)EditorWindow.GetWindow(typeof(LevelEditToolsWindow), false, "LevelEdit Window");
        window.Show();
    }
	...
	void OnGUI () {
		...
		SerializedProperty levelListProp = so.FindProperty("levelList");

		scrollPos = EditorGUILayout.BeginScrollView (scrollPos, false, false);
		GUILayout.Label("Upgrade levels", EditorStyles.boldLabel);

		GUILayout.Label("Level(s) to upgrade", EditorStyles.miniBoldLabel);
		EditorGUILayout.PropertyField(levelListProp, true);
		...
		foreach (var level in levelList) {
			...
			foreach (var tile in tileList) {
				...
				if (tile.TileDef.type == TileType.Bucket) {
					var def = new PaintBucketDefinition { pos = pos, className = "PaintBucket", color = tile.TileDef.color };
					level.CreateTOAtPos(typeof(PaintBucket), def);
				}else if (tile.TileDef.type == TileType.Start) {
					var def = new EntranceDefinition { pos = pos, className = "Entrance" };
					level.CreateTOAtPos(typeof(Entrance), def);
				}else if (tile.TileDef.type == TileType.Goal) {
					var def = new ExitDefinition { pos = pos, className = "Exit" };
					level.CreateTOAtPos(typeof(Exit), def);
				}

				TileDefinition tileDef = new TileDefinition { type = TileType.Normal, goalColor = TileColor.None, color = TileColor.None, pos = pos };
